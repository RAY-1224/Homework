...
本題使用 無向圖 Undirected Graph，並實作兩種基本的圖形走訪方法：
...
### 選擇Binary Search Tree（BST）的原因


1. **採用 遞迴（Recursive）實作**
   
    因為每個節點都可以當作一棵「子樹」，寫起來比較直觀

3. **BST 本身就有「左小右大」的特性**
   
    可以幫助我們在操作後，還能維持正確的排序關係
   
5. **透過中序走訪（Inorder Traversal）**
   
   可以快速確認樹是否還是正確的 BST（會呈現排序結果）
